  ...
  implements BasicController, Runnable
{
    ...
    throws BasicPlayerException
  {
    try
    {
      ...
      m_status = OPENED;
      notifyEvent(BasicPlayerEvent.OPENED, getEncodedStreamPosition(), -1,
                  null);
    }
    ...
    throws LineUnavailableException
  {
    ...
    if (m_line == null)
    {
      ...
      m_audioInputStream =
          AudioSystem.getAudioInputStream(targetFormat, m_audioInputStream);
  ...
  protected void pausePlayback()
  {
    if (m_line != null)
    {
      if (m_status == PLAYING)
      {
        ...
        notifyEvent(BasicPlayerEvent.PAUSED, getEncodedStreamPosition(),
                    -1, null);
      }
    }
  }
  ...
  protected void resumePlayback()
  {
    if (m_line != null)
    {
      if (m_status == PAUSED)
      {
        ...
        notifyEvent(BasicPlayerEvent.RESUMED, getEncodedStreamPosition(),
                    -1, null);
      }
    }
  }
    ...
    throws BasicPlayerException
  {
    ...
    if (m_status == OPENED)
    {
      ...
      {
        throw new BasicPlayerException(BasicPlayerException.CANNOTINITLINE,
                                       e);
      }
      ...
      if (m_line != null)
      {
        m_line.start();
        m_status = PLAYING;
        notifyEvent(BasicPlayerEvent.PLAYING, getEncodedStreamPosition(),
                    -1, null);
      }
    }
  }
  ...
  public void run()
  {
    ...
    synchronized (m_audioInputStream)
    {
      ...
      if (nBytesRead == -1)
      {
        notifyEvent(BasicPlayerEvent.EOM, getEncodedStreamPosition(), -1,
                    null);
      }
      // Close stream.
      closeStream();
    ...
    throws BasicPlayerException
  {
    ...
    if (m_dataSource instanceof File)
    {
      ...
      try
      {
        synchronized (m_audioInputStream)
        {
          notifyEvent(BasicPlayerEvent.SEEKING, getEncodedStreamPosition(),
                      -1, null);
          initAudioInputStream();
          if (m_audioInputStream != null)
          {
        ...
        notifyEvent(BasicPlayerEvent.SEEKED, getEncodedStreamPosition(),
                    -1, null);
        m_status = OPENED;
  ...
  protected void notifyEvent(int code, int position, double value,
                             Object description)
  {
    BasicPlayerEventLauncher trigger =
      new BasicPlayerEventLauncher(code, position, value, description,
    ...
    throws BasicPlayerException
  {
    if (hasGainControl())
    {
      ...
      double valueDB =
        minGainDB + (1 / cste) * Math.log(1 + (Math.exp(cste * ampGainDB) -
                                               1) * fGain);
      log.debug("Gain : " + valueDB);
      m_gainControl.setValue((float) valueDB);
      notifyEvent(BasicPlayerEvent.GAIN, getEncodedStreamPosition(), fGain,
                  null);
    }
    else
